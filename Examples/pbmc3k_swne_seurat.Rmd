...
Pull out the counts, scale and adjust gene variance, while keeping the scaled matrix nonnegative.
```{r}
...
var.genes <- intersect(se.obj@var.genes, rownames(norm.counts));
length(var.genes)
...
k.err <- FindNumFactors(norm.counts[var.genes,], k.range = k.range, n.cores = n.cores, do.plot = T, loss = loss)
```
