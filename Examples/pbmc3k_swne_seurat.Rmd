...
Makes the initial SWNE plot. We set a seed so that the cluster colors are reproducible across different plots.
...
Create a t-SNE plot for comparison
```{r fig.width=5,fig.height=5}
...
We'll remake the SWNE plot, this time showing the embedded genes. The closer a cell or a cluster is to a gene, the higher the expression level.
```{r fig.width=5,fig.height=5}
PlotSWNE(swne.embedding, alpha.plot = 0.4, sample.groups = cell.clusters, do.label = T,
         label.size = 3.5, pt.size = 1.25, show.legend = F, seed = color.seed)
```
...
We can also make a heatmap to visualize the top factors for each gene
