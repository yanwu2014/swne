...
```{r}
## Run SWNE
obj <- RunSWNE(obj, k = 16, alpha.exp = 1.5, snn.exp = 0.25, 
               var.genes = var.genes, genes.embed = genes.embed,
...
snn <- FindNeighbors(obj, k = 10, prune.SNN = 1/15)
...
```{r message=FALSE,warning=FALSE}
alpha.exp <- 1.5 # Increase this > 1.0 to move the cells closer to the factors. Values > 2 start to distort the data.
...
swne.embedding <- EmbedSWNE(nmf.res$H, snn, alpha.exp = alpha.exp, snn.exp = snn.exp,
                            n_pull = n_pull)
```
