...
se.obj <- RunTSNE(se.obj, dims.use = 1:16, perplexity = 20)
se.obj <- BuildSNN(se.obj, dims.use = 1:16, k.param = 30, k.scale = 10, prune.SNN = 1/20, force.recalc = T)
...
k.res <- FindNumFactors(norm.counts[se.obj@var.genes,], k.range = k.range, n.cores = n.cores, na.frac = 0.25,
                        seed = 32590, do.plot = T, loss = loss, recon.err = recon.err)
k.res$k
...
swne.embedding <- EmbedSWNE(nmf.scores, SNN = snn.matrix, alpha.exp = 2.0, snn.exp = 0.1, n_pull = 4, dist.use = "IC")
...
PlotSWNE(swne.embedding, alpha.plot = 0.5, sample.groups = clusters, do.label = T,
         label.size = 3.5, pt.size = 1.5, show.legend = F, seed = seed) + 
...
PlotDims(tsne.scores, sample.groups = clusters, show.legend = F, show.axes = F, 
         alpha.plot = 0.5, label.size = 3.5, pt.size = 1.5, seed = seed) + 
  scale_color_manual(values = cluster_colors)
dev.off()
...
PlotDims(tsne.scores, sample.groups = clusters, show.legend = F, show.axes = F, 
         alpha.plot = 0.5, pt.size = 1.5, label.size = 0, seed = seed) + 
  scale_color_manual(values = cluster_colors)
dev.off()
...
cairo_pdf("hemato_Sun2_feature_plot.pdf", width = 4.5, height = 4.5)
